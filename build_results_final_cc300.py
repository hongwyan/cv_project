...
def parse_args():
    parser = argparse.ArgumentParser(description="Build pooled final table for cc300 postprocessed 3D results.")
...
def summarize_cc300(df: pd.DataFrame) -> pd.DataFrame:
    rows = []
...
def main():
    ...
    summary_df = summarize_cc300(df)
    display_df = format_display(summary_df)

    out_path = output_dir / "pooled_results_cc300.csv"
