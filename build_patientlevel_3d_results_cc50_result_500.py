...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Build patient-level 3D Dice/HD95 metrics for result_500 after removing connected components smaller than a chosen voxel threshold."
    )
...
def remove_small_components_3d(pred: np.ndarray, min_size: int) -> np.ndarray:
...
def build_summary(patient_df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for (seed, model_name), group_df in patient_df.groupby(["seed", "model"], sort=True, observed=False):
        ...
        rows.append(
            {
                ...
                "n_patients_used_for_dice_hd95": int(dice_mask.sum()),
...
def main():
    ...
    suffix = f"cc{args.min_component_size}"
    output_dir = Path(args.output_dir) if args.output_dir else results_root / "summary_0.5" / f"results_3D_{suffix}"
    ...
    for dataset_seed in range(args.seed_start, args.seed_end + 1):
        ...
        for patient_id in reference_patients:
            ...
            for key, config in configs.items():
                ...
                pred = remove_small_components_3d(pred_raw, min_size=args.min_component_size)
    ...
    patient_df_rounded = round_float_columns(patient_df, {"seed", "patient_id", "model", "gt_nonempty", "pred_nonempty"})
    summary_df_rounded = round_float_columns(summary_df, {"seed", "model", "n_patients_total", "n_patients_used_for_dice_hd95"})

    patient_csv = output_dir / f"patient_metrics_3d_{suffix}_by_seed.csv"
    summary_csv = output_dir / f"patient_metrics_3d_{suffix}_mean_std_by_seed.csv"
    summary_json = output_dir / f"patient_metrics_3d_{suffix}_mean_std_by_seed.json"
    summary_md = output_dir / f"patient_metrics_3d_{suffix}_mean_std_by_seed_3sf.md"
    shape_json = output_dir / "patient_volume_shapes.json"

    patient_df_rounded.to_csv(patient_csv, index=False, encoding="utf-8", float_format="%.3f")
