...
def parse_args():
    parser = argparse.ArgumentParser(description="Build pooled Wilcoxon test table for cc500 boundary comparisons.")
...
def main():
    ...
    for lhs_model, rhs_model in COMPARISONS:
        for metric in METRICS:
            ...
            _, pvalue = safe_wilcoxon(lhs_vals[mask], rhs_vals[mask])
            rows.append(
                {
                    "lhs_model": lhs_model,
                    "rhs_model": rhs_model,
    ...
    csv_path = output_dir / "pooled_results_cc500_wilcoxon.csv"
    md_path = output_dir / "pooled_results_cc500_wilcoxon_3sf.md"
